...
import { ImagePlus, Pencil, RefreshCw, Search } from 'lucide-react'
import { useNavigate } from 'react-router-dom'
import PageHeader from '../components/PageHeader'
import { listPhotography, type PhotographyListItem, type PhotographyStatus } from '../api/photography'
    ...
    <div className="page-stack">
      ...
      <section className="surface-panel list-panel">
        <div className="list-toolbar">
          <Segmented options={['全部', '草稿', '已发布']} value={filter} onChange={(value) => setFilter(String(value))} />
          <Input className="compact-search" allowClear prefix={<Search size={16} />} placeholder="搜索摄影集" value={query} onChange={(event) => setQuery(event.target.value)} onPressEnter={() => void load()} />
          <Button icon={<RefreshCw size={16} />} aria-label="刷新" onClick={() => { setLoading(true); void load() }} />
        </div>
        <Spin spinning={loading}>{items.length === 0 ? <div className="media-empty">还没有摄影集，创建第一组照片故事吧。</div> : <div className="photography-admin-list">{items.map((item) => <article className="photography-admin-item" key={item.id}><div className="photography-admin-item__cover">{item.coverPublicUrl && <img src={item.coverPublicUrl} alt="" />}</div><div className="photography-admin-item__body"><div><Tag color={item.status === 'PUBLISHED' ? 'green' : 'default'}>{item.status === 'PUBLISHED' ? '已发布' : item.status === 'WITHDRAWN' ? '已撤回' : '草稿'}</Tag><span>{item.photoCount} 张照片</span></div><h2>{item.title}</h2><p>{item.locationText || '未设置地点'} · {item.updatedAt.slice(0, 10)}</p></div><Button type="text" icon={<Pencil size={16} />} aria-label={`编辑${item.title}`} onClick={() => navigate(`/photography/${item.id}/edit`)} /></article>)}</div>}</Spin>
      </section>
    </div>
  )
}
